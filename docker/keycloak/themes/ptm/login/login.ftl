...
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('username','password') displayInfo=realm.password && realm.registrationAllowed && !registrationDisabled??; section>
    <#if section = "form">
                ...
                <form class="mt-4" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
                    ...
                    <button tabindex="4" type="submit" class="btn btn-ptm-primary mt-3 w-100">${msg("doLogIn")}</button>

                    <!-- Register -->
                    <#if realm.password && realm.registrationAllowed && !registrationDisabled??>
                    <a href="${url.registrationUrl}" class="btn btn-ptm-secondary w-100 mt-2">${msg("doRegister")}</a>
                    </#if>
                </form>
